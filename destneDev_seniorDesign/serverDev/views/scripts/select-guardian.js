    ...
    if(json.hasOwnProperty('error')) {
      ...
      for (var i = 0; i < json.users.length; i++) {
        ...
        if(json.users[i].username !== $('#usrName').text()) {
          tr.append('<td><a class="btn btn-default" href="/client/manage-guardian/' + json.users[i].userId + '"><i class="fa fa-pencil-square-o"></i> Edit Profile</a><button type="button" class="btn btn-danger delete-guardian-modal" data-toggle="modal" data-target="#delete"><i class="fa fa-user-times"></i>	Delete </a></td>');
        }
        else {
          tr.append('<td><a class="btn btn-default" href="/client/manage-guardian/' + json.users[i].userId + '"><i class="fa fa-pencil-square-o"></i> Edit Profile</a></td>');
        }
        $('#userTable').append(tr);
      }
    }
